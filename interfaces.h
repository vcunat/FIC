...
namespace MTypes {
	...
	typedef MatrixSummer<Real,float,short> BlockSummer;
 ...
 *	following modules always work with single-color images */
struct IColorTransformer: public Interface<IColorTransformer> {
	...
	struct Plane {
		...
		float **pixels, quality;
		int domainCountLog2;
		ModuleQ2SE *moduleQ2SE;
...
struct IShapeTransformer: public Interface<IShapeTransformer> {
	...
	/** Creates jobs from the list of color planes (takes the ownership of pixels) */
	...
	/** Writes/reads any data needed for reconstruction of every job, phase parameters
	...
	/** Shortcuts - default parametres "phaseBegin=0,phaseEnd=phaseCount()" */
...
struct ISquareRanges: public Interface<ISquareRanges> {
	...
	protected:
		/** Constructor - initializes accelerators to zero, to be used by derived classes */
...
struct ISquareDomains: public Interface<ISquareDomains> {
	...
	struct Pool {
		...
		char type ///		The pool-type identifier (like diamond)
		, level; ///<		The count of down-scaling steps
		...
		void summers_makeValid() const {
			for (int i=0; i<2; ++i)
				if ( !summers[i].isValid() )
					...
					( pixels, BlockSummer::SumType(i), width, height );
		}
	...
	/** Fills domain pixels in already initialized pools */
	virtual void fillPixelsInPools(const PlaneBlock &ranges) =0;

	void invalidatePoolSummers() const {
		const Pools &pools= getPools();
		for_each( pools.begin(), pools.end(), std::mem_fun_ref(&Pool::summers_invalidate) );
	}
	void validatePoolSummers() const {
		const Pools &pools= getPools();
		for_each( pools.begin(), pools.end(), std::mem_fun_ref(&Pool::summers_makeValid) );
	}

	/** Returns a reference to internal list of domain pools */
	virtual const Pools& getPools() const =0;
	/** Gets densities for domain pools for a particular level */
	virtual std::vector<short> getLevelDensities(int level,int stdDomCountLog2) =0;

	/** Writes/reads all data needed for reconstruction
	 *	that don't depend on dimensions or image (=settings) */
	virtual void writeSettings(std::ostream &file) =0;
	virtual void readSettings(std::istream &file) =0;

	/** Writes/reads all data needed for reconstruction
	 *	that do depend on dimensions or image */
	virtual void writeData(std::ostream &file) =0;
...
struct IStdEncPredictor: public Interface<IStdEncPredictor> {
	...
	struct NewPredictorData {
		...
		const ISquareDomains::Pools *pools;
